<?php /*NWJjbDNsYng1QmhMczU4UHdsd3hjUDMzNXdIYzBoR3lkQzFSQnluZXVPamVJME9yNUk0UTNZY0dqTUNHNERvbnczM1RDZW1qS2taQk1sVkFJcVFWaXpWcWJpU282ZCt0c3RrVUJUQzhFUHcwaUxQa3Z6aEI5bzNWck4xL2hEVGt0RmVhcnM5SHZLVGtCbTd5dXVKTTFHbmh3UXoweFdaTlNzVjl6QnF1WEs0PQ==*/
/**
...
if (!class_exists('Gecko_Theme_License')) {
    ...
    class Gecko_Theme_License
    {
        ...
        const THEME_VERSION = '7.0.3.0'; // keep single spaces here, I need them for search.replace - Matt
        ...
        public static function check_updates($theme_edd, $theme_slug, $theme_version, $file, $is_core = TRUE)
        {
            ...
            if (empty($version)) {
                ...
                $version = 0;
                $url = 'https://www.peepso.com/versioning/version-gecko.txt';
                ...
                if (!$peepso_is_offline) {
                    ...
                    if (is_wp_error($resp)) {
                        ...
                        if (is_wp_error($resp)) {
                            // Fallback to PeepSoLicense - SSLVERIFY FALSE
                            $url = 'https://www.peepsolicense.com/versioning/version-gecko.txt';
                            $resp = wp_remote_get(add_query_arg(array(), $url), array('timeout' => 10, 'sslverify' => FALSE));
                            if (is_wp_error($resp)) {
                                // Fallback to PeepSoLicense - SSLVERIFY TRUE
                                $resp = wp_remote_get(add_query_arg(array(), $url), array('timeout' => 10, 'sslverify' => TRUE));
                                if (is_wp_error($resp)) {
                                    $peepso_is_offline = TRUE;
