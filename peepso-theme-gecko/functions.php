<?php /**/
...
if(class_exists('PeepSo')) {
    ...
    while(count($ver_peepso) > $limit) {
        array_pop($ver_peepso);
    }

    while(count($ver_gecko) > $limit) {
        array_pop($ver_gecko);
    }
...
function gecko_load_theme_setup() {
    ...
    // Language
    $path = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'language' . DIRECTORY_SEPARATOR . 'peepso-theme-gecko-' . determine_locale() . '.mo';
    load_textdomain( 'peepso-theme-gecko', $path );
...
function guest_redirect() {
    ...
    if(!apply_filters('gecko_filter_should_guest_redirect', true)){
        return false;
    }

    // Get settings
    $gecko_settings = GeckoConfigSettings::get_instance();
    $redirect_page_id = apply_filters('gecko_filter_opt_redirect_guest',$gecko_settings->get_option( 'opt_redirect_guest', 0 ));
...
//
if ( ! function_exists( 'gecko_elementor_register_elementor_locations' ) ) {
    /**
     ...
     * @return void
     */
    function gecko_elementor_register_elementor_locations( $elementor_theme_manager ) {
        $gecko_settings = GeckoConfigSettings::get_instance();
        $elementor_support = $gecko_settings->get_option( 'opt_elementor_headerfooter' );

        if ( $elementor_support && apply_filters( 'gecko_elementor_register_elementor_locations', true ) ) {
            $elementor_theme_manager->register_all_core_location();
        }
    }
}
add_action( 'elementor/theme/register_locations', 'gecko_elementor_register_elementor_locations' );
