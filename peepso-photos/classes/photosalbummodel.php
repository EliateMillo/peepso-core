<?php /**/
...
class PeepSoPhotosAlbumModel
{
    ...
    public function get_num_photos_by_album($user_id, $album_id='', $module_id = 0)
    {
        ...
        if (PeepSo::get_option_new('postbox_anon_enabled')) {
			$clauses['join'] .=
            " LEFT JOIN `{$wpdb->postmeta}` ON `" . $wpdb->prefix . PeepSoPhotosModel::TABLE . "`.`pho_post_id`=`{$wpdb->postmeta}`.`post_id` AND `{$wpdb->postmeta}`.`meta_key` = '".PeepSo3_Anon::META_POST_ANON_OP."'";
			$clauses['where'] .=
