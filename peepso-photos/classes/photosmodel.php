<?php /**/
...
class PeepSoPhotosModel
{
	...
	public function save_images(Array $files, $post_id, $act_id, $album_id = 0)
	{
		...
		foreach ($files as $file) {
			...
			// Copy to anonymous folder
			$anon_id = $input->int('anon_id');
			if ($anon_id > 0) {
				$user = PeepSoUser::get_instance($anon_id);
				$image_dir_anon = ($user) ? $user->get_image_dir() : '';
				$image_dir_anon .= 'photos' . DIRECTORY_SEPARATOR;
				if (!is_dir($image_dir_anon)) {
					mkdir($image_dir_anon, 0755, true);
				}

				$dest_file_anon = $image_dir_anon . $filename;
				copy($dest_file, $dest_file_anon);
				
				$files_to_save_anon = array($dest_file_anon);
				$temp_anon=wp_get_image_editor($dest_file_anon);
				$thumbs_new_anon = $this->generate_thumbs_anon($anon_id, $filename, $filetype['ext'], $temp_anon, NULL);
				$files_to_save_anon = array_merge($files_to_save_anon, $thumbs_new_anon);
			}
			...
			if ($enable_aws_s3) {
				...
				$j=0;
                foreach ($files_to_save_anon as $key=> $file) {
                    $pho_token = $this->upload_to_amazon_s3($file);
                    if (NULL !== $pho_token) {
						$aws_fallback = false;
...
//			}
	...
	public function generate_thumbs_anon($anon_id, $filename, $filetype, $image, $required=NULL) {
        $filename = str_replace('.'.$filetype, '', $filename);

		$user = PeepSoUser::get_instance($anon_id);
		$image_dir = ($user) ? $user->get_image_dir() : '';
		$image_dir .= 'photos'. DIRECTORY_SEPARATOR . 'thumbs' . DIRECTORY_SEPARATOR;
        if (!is_dir($image_dir)) {
            mkdir($image_dir, 0755, TRUE);
		}
    ...
    public function get_post_photos($post_id, $order='asc')
    {
        ...
        if (!empty($photos)) {
			...
			if(strlen($anon_op)) {
				$author = PeepSo3_Anon::get_instance()->anon_id;
			}
            $user = PeepSoUser::get_instance($author);
    ...
    public function get_user_photos_by_album($user_id, $album_id, $offset = 0, $limit = 10, $sort = 'desc', $module_id = 0)
    {
		...
		if (PeepSo::get_option_new('postbox_anon_enabled')) {
			$clauses['join'] .=
            " LEFT JOIN `{$wpdb->postmeta}` ON `" . $wpdb->prefix . self::TABLE . "`.`pho_post_id`=`{$wpdb->postmeta}`.`post_id` AND `{$wpdb->postmeta}`.`meta_key` = '".PeepSo3_Anon::META_POST_ANON_OP."'";
			$clauses['where'] .=
