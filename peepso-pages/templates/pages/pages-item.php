<div class="ps-page ps-js-page-item ps-js-page-item--{{= data.id }} {{= data.published ? '' : 'ps-page--unpublished' }}">
    <div class="ps-page__inner">
        <a href="{{= data.url }}" class="ps-page__header" style="background-image:url('{{= data.cover_url }}');">
            <div class="ps-avatar ps-avatar--page">
                <img src="{{= data.avatar_url_full }}" alt="">
            </div>
        </a>

        <div class="ps-page__body">
            <div class="ps-page__name">
                <a href="{{= data.url }}">{{= data.nameHighlight }}</a>
            </div>

            <div class="ps-page__details">
                <div class="ps-page__desc">
                    <p>{{= data.description }}</p>
                </div>
                <?php /**/
                #6666 GeoMyWp hooks
                do_action('peepso_action_render_pages_item_details_before');
                ?>
                <div class="ps-page__details-item">
                    <i class="gcis gci-user-friends"></i>
                    <span class="ps-js-member-count">
            {{= data.members_count }} {{= data.members_count > 1 ? '<?php echo __("followers", "pageso"); ?>' : '<?php echo __("follower", "pageso"); ?>' }}
            {{ if ( +data.pending_admin_members_count >= 1 ) { }}
            ({{- '<?php echo __("%d pending", "pageso"); ?>'.replace( '%d', data.pending_admin_members_count ) }})
            {{ } }}
          </span>
                </div>
                <?php
                #6666 GeoMyWp hooks
                do_action('peepso_action_render_pages_item_details_after');
                ?>

                <?php $show_more = FALSE; ?>

                <?php if (intval(PeepSo::get_option('pages_listing_show_page_owner',1))) { ?>
                    <?php $show_more = TRUE; ?>
                    <div class="ps-page__details-item ps-page__details-item--hide">
                        <i class="gcis gci-user-circle"></i><span class="ps-js-owner" data-id="{{= data.id }}"><img src="<?php echo PeepSo::get_asset('images/ajax-loader.gif') ?>" /></span>
                    </div>
                <?php } ?>

                <?php if (intval(PeepSo::get_option('pages_listing_show_page_creation_date',1))) { ?>
                    <?php $show_more = TRUE; ?>
                    <div class="ps-page__details-item ps-page__details-item--hide">
                        <i class="gcis gci-clock"></i><span>{{= data.date_created_formatted }}</span>
                    </div>
                <?php } ?>

                <?php if (PeepSo::get_option('pages_categories_enabled', FALSE)) { ?>
                    <?php $show_more = TRUE; ?>
                    <div class="ps-page__details-item ps-page__details-item--hide">
                        <i class="gcis gci-tag ps-js-category-icon"></i><span class="ps-js-categories" data-id="{{= data.id }}"><img src="<?php echo PeepSo::get_asset('images/ajax-loader.gif') ?>" /></span>
                    </div>
                <?php } ?>

                <?php if ($show_more) { ?>
                    <a href="#" class="ps-page__details-item ps-page__details-item--more ps-link--more ps-js-more">
                        <i class="gcis gci-info-circle"></i>
                        <span><?php echo __('More', 'pageso'); ?></span>
                    </a>
                <?php } ?>
            </div>
        </div>
    </div>
    <div class="ps-page__actions ps-js-member-actions">{{= data.member_actions }}</div>
</div>
