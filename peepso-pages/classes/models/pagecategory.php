<?php /**/

class PeepSoPageCategory
{
	...
	const POST_TYPE 		= 'peepso-page-cat';

	private $_table;

	/**
	 * PeepSoPageCategory constructor
	 * @param $id
	 */
	public function __construct($id = NULL, $data = NULL)
	{
			...
			if(isset($data['meta'])) {
				...
				if(is_array($meta)) {
					foreach($meta as $key=>$val) {
						add_post_meta($this->id, 'peepso_page_cat_' . $key, $val, TRUE);
					}
				}

				unset($data['meta']);
			}
			// Default the "order" to page_cat_id, ensures the new page category will be on the bottom
			$data['order'] = $this->id;
			// force open new category
			update_user_meta(get_current_user_id(), 'peepso_admin_page_category_open_' . $this->id, 1);
			$this->update($data);

			do_action('peepso_action_page_category_create', $this);
		}

		if(-1 == $id) {
			$this->id = -1;
			$this->name = __('Uncategorized', 'pageso');
			return;
		}
		...
		foreach($this->meta_data_map as $property=>$default) {
			$this->$property = get_post_meta($this->id, 'peepso_page_cat_'.$property, TRUE);
			// get_post_meta WILL RETURN AN EMPTY STRING if the key is not found
			if('' === $this->$property) {
				add_post_meta($this->id, 'peepso_page_cat_' . $property, $default, TRUE);
		...
		if(!strlen($this->slug) && $this->id != -1) {
            ...
            if(!$name) {
                $name = 'page-category-'.$this->id;
            }

            $new_slug = wp_unique_post_slug(sanitize_title_with_dashes($name), $this->id, 'any', 'peepso-page-cat', 0);
		...
		foreach( $data as $key=>$value ) {
			...
			if( property_exists($this, $key )) {
				...
				if(array_key_exists($key, $this->meta_data_map)) {
					update_post_meta($this->id, 'peepso_page_cat_' . $key, $value);
					continue;
				}
	...
	public function get_url($segment = '')
	{
        // Uncategorized
        if(empty($this->slug)) {
            return PeepSo::get_page('pages') . '?category=' . $this->id;
        }
        ...
        return PeepSo::get_page('pages') . '?category/' . $this->slug.'/'.$segment;
	}
    ...
    {
        $avatar_hash = get_post_meta($this->id,'peepso_page_category_avatar_hash', TRUE);
        if($avatar_hash) {
    ...
    /**
     * Return default avatar for page category.
     *
     * @param boolean $full
     * @return string
     */
    public function get_default_avatar_url($full = TRUE) {
        return PeepSo::get_asset('images/avatar/page-category' . ($full ? '' : '-thumb') . '.png');
    }

    function set_avatar_custom_flag( $flag = TRUE )
    {
        $current_val = get_post_meta($this->id, 'page_category_avatar_custom', TRUE);
        ...
        update_post_meta($this->id, 'page_category_avatar_custom', $flag);
    ...
    public function has_avatar()
    {
        ...
        if (strpos($avatar, '/page-categories/') !== FALSE) {
            return (TRUE);
        }
        return (FALSE);
    }
    ...
    public function finalize_move_avatar_file()
    {
        ...
        update_post_meta($this->id,'peepso_page_category_avatar_hash', $avatar_hash);
    ...
    /*
     * Return the page category's cover photo
    ...
    public function get_cover_url()
    {
        ...
        $page_category_cover_photo = get_post_meta($this->id, 'page_category_cover_photo', TRUE);
        if (!empty($page_category_cover_photo)) {
            $cover = $page_category_cover_photo;
        }
    ...
    /**
     * Get current page category's default cover image.
     *
     * @return string
     */
    public function get_cover_default()
    {
        return PeepSo::get_asset('images/cover/page-category-default.png');
    }

    /*
     * Check if page category has uploaded a custom cover photo
    ...
    public function has_cover()
    {
        ...
        if (FALSE !== stripos($cover, '/page-categories/')) {
            return (TRUE);
        }
        return (FALSE);
    }
    ...
    /**
     * Display the current page category's cover photo position percentage.
     */
    ...
    public function move_cover_file($src_file)
    {
        ...
        update_post_meta($this->id,'peepso_page_category_cover_hash', $hash);
        ...
        update_post_meta($this->id, 'page_category_cover_photo', $this->get_image_url() . $hash . '-cover.jpg');
    ...
    public function delete_cover_photo($cover_hash = '')
    {
        ...
        if (!$cover_hash) {
            $cover_hash = get_post_meta($this->id,'peepso_page_category_cover_hash', TRUE);
        ...
        delete_post_meta($this->id, 'page_category_cover_photo');
        delete_post_meta($this->id, 'peepso_cover_position_x');
        delete_post_meta($this->id, 'peepso_cover_position_y');
...
//			'pages' . DIRECTORY_SEPARATOR . $this->id . DIRECTORY_SEPARATOR;
        $dir = PeepSo::get_peepso_dir() . 'page-categories' . DIRECTORY_SEPARATOR . $this->id . DIRECTORY_SEPARATOR;
...
//		$dir = WP_CONTENT_URL . '/peepso/pages/' . $this->id . '/';
        $dir = PeepSo::get_peepso_uri() . 'page-categories/' . $this->id . '/';
        return ($dir);
    }
