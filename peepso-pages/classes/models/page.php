...
class PeepSoPage
{
    ...
    public $privacy; // PeepSoPagePrivacy
    public $is_open		= TRUE;
    ...
    private $_pageuser = FALSE;

    const POST_TYPE 		= 'peepso-page';

    /**
     * PeepSoPage constructor.
     * @param $id
     */
    public function __construct($id = NULL, $data = NULL)
    {
        ...
        $this->meta_data_map = array(
            'members_count' 				=> 1, // the page creator should always be a member, so the minimum member count is "1"
            'pending_admin_members_count'	=> 0,
            'pending_user_members_count'	=> 0,
            'banned_members_count'			=> 0,
            'privacy'						=> PeepSoPagePrivacy::PRIVACY_OPEN, // pages are public by default,
            'rules'                         => '',
            'is_joinable'					=> TRUE,
            'is_invitable'					=> TRUE,
            'is_readonly'					=> FALSE,
            'members_tab'					=> PeepSo::get_option_new('pages_members_tab'),
        ...
        if($id instanceof PeepSoPage) {
            $id = $id->get('id');
        }

        // constructor is also able to handle page creation
        if( NULL == $id && is_array($data) ) {

            // optional filters to modify data before creation
            // $data = apply_filters('peepso_pages_will_create_page', $data);

            // If no owner_id was passed, default to current user and generate a notice
            if( !isset($data['owner_id']) ) {
                trigger_error("Missing owner_id PeepSoPage", E_USER_NOTICE);
            ...
            if(PeepSo::get_option('pages_categories_enabled', FALSE)) {
                if (isset($data['category_id'])) {
                    PeepSoPageCategoriesPages::add_page_to_categories($id, $data['category_id']);
                    unset($data['category_id']);
                } else {
                    trigger_error("Missing category_id PeepSoPage", E_USER_NOTICE);
                }
            }
            ...
            if(isset($data['meta'])) {
                ...
                if(is_array($meta)) {
                    foreach($meta as $key=>$val) {
                        add_post_meta($this->id, 'peepso_page_' . $key, $val, TRUE);
                    }
                }
            ...
            // hooks after pages created
            do_action('peepso_action_page_create', $this);
        }

        // grabbing page by numeric id...
        $args = array(
            'include'	=> array($id),
        ...
        foreach($this->meta_data_map as $property=>$default) {
            $this->$property = get_post_meta($this->id, 'peepso_page_'.$property, TRUE);
            // get_post_meta WILL RETURN AN EMPTY STRING if the key is not found
            if('' === $this->$property) {
                add_post_meta($this->id, 'peepso_page_' . $property, $default, TRUE);
                $this->$property = $default;
            }
        }

        switch($this->privacy) {
            case PeepSoPagePrivacy::PRIVACY_CLOSED:
                $this->is_open 	 = FALSE;
                $this->is_closed = TRUE;
                $this->is_secret = FALSE;
                break;
            case PeepSoPagePrivacy::PRIVACY_SECRET:
                $this->is_open	 = FALSE;
        ...
        $this->privacy = PeepSoPagePrivacy::_($this->privacy);
        ...
        if(!strlen($this->slug)) {
            ...
            $new_slug = wp_unique_post_slug(sanitize_title_with_dashes($name), $this->id, 'any', 'peepso-page', 0);
        ...
        if(is_numeric($this->slug)) {
            $new_slug = wp_unique_post_slug(sanitize_title_with_dashes('page-'.$this->id), $this->id, 'any', 'peepso-page', 0);

            $this->update(array('slug' => $new_slug));
        }

        // Fallback for blocked slug
        if(in_array($this->slug, PeepSoPagesPlugin::$page_slug_blocklist)) {
            $new_slug = 'page-'.$this->slug;
            $new_slug = wp_unique_post_slug(sanitize_title_with_dashes($new_slug), $this->_page_id, 'any', 'peepso-page', 0);
            $this->update(array('slug' => $new_slug));
        }
    ...
    /**
     * Singleton - if the PeepSoPageUser instance is needed, try to load it only once
     * @return bool|PeepSoPage
     */
    private function get_pageuser_instance()
    {
        if(FALSE === $this->_pageuser) {
            $this->_pageuser = new PeepSoPageUser($this->id, NULL, $this);
        }

        return $this->_pageuser;
    }
     ...
     * @todo and runs other page creation tasks
    ...
    private function create($name = NULL)
    {
        ...
        // @todo add the creator as a page member
        $page_user = new PeepSoPageUser($id);
        $page_user->member_join();
        $page_user->member_modify('member_owner');
     ...
     * @param array $data - key/value array of page properties
     */
        ...
        foreach( $data as $key=>$value ) {
            ...
            if( property_exists($this, $key )) {
                ...
                if(in_array($key, $this->meta_data_map)) {
                    update_post_meta($this->id, 'peepso_page_' . $key, $value);
                    continue;
                }

            } else {
                trigger_error("Unknown property PeepSoPage::$key", E_USER_NOTICE);
            }
        }
    ...
    public static function validate($page_data)
    {
        $errors=array();
        ...
        foreach($page_data as $property=>$value) {
            ...
            if(is_array($value)) {
                if((!count($value) && TRUE == $rule['required'])) {
                    $errors[$property] = __('This field is required', 'pageso');
                }
            } else {
            ...
            if ( isset($rule['not_int']) && is_numeric($value) && TRUE == $rule['not_int'] ) {
                $errors[$property] = __('This field can\'t be a number', 'pageso');
            }
            ...
            if ( isset($rule['minlength']) && is_numeric($rule['minlength'])  && strlen(PeepSo3_Utilities_String::text_to_html_entities($value, ' ')) < $rule['minlength']) {
                $errors[$property] = sprintf( __('Minimum length is %s characters', 'pageso'), $rule['minlength'] );
            }
            ...
            if ( isset($rule['maxlength']) && is_numeric($rule['maxlength'])  && strlen(PeepSo3_Utilities_String::text_to_html_entities($value, ' ')) > $rule['maxlength'] ) {
                $errors[$property] = sprintf(__('Maximum length is %s', 'pageso'), $rule['maxlength']);
            }

            // #5493 let admin enforce unique page names
            if( 'name' == $property && PeepSo::get_option_new('pages_unique_names') ) {

                global $wpdb;
                $value=str_replace(' ','',trim(strtoupper($value)));
                $sql = "SELECT COUNT(id) FROM {$wpdb->posts} WHERE REPLACE(UPPER(post_title),' ','') = '$value' AND `post_type`='peepso-page'";

                if( isset($page_data['id']) && is_int($page_data['id'] ) ) {
                    $sql .= " AND ID <> {$page_data['id']}";
                }

                $count = $wpdb->get_var($sql);

                if($count > 0) {
                    $errors[$property] = sprintf(__('This page name cannot be used', 'pageso'), $rule['maxlength']);
                }
            }
        }
    ...
    public function get_url( $permalink = FALSE )
    {
        ...
        if( TRUE === $permalink || (0 == PeepSo::get_option('pages_urls_slugs_enable', 1))) {
            $url[]= $this->id;
        } else {
            $url[] = $this->slug;
        }
    ...
    /*
     * Obtain href for page's full sized avatar image
     *
     */
    ...
    /**
     * Display the page's original avatar
     */
    ...
    {
        $avatar_hash = get_post_meta($this->id,'peepso_page_avatar_hash', TRUE);
        if($avatar_hash) {
    ...
    /**
     * Return default avatar for page.
     *
     * @param boolean $full
     * @return string
     */
    public function get_default_avatar_url($full = TRUE) {
        return PeepSo::get_asset('images/avatar/page' . ($full ? '' : '-thumb') . '.png');
    }

    function set_avatar_custom_flag( $flag = TRUE )
    {
        $current_val = get_post_meta($this->id, 'page_avatar_custom', TRUE);
        ...
        update_post_meta($this->id, 'page_avatar_custom', $flag);

    }

    /*
     * Return page's temporary avatar image
     * @param boolean $full TRUE to return full-size image or FALSE for small image
     * @return string The href value for the avatar image suitible for use in an <img> tag
     */
    ...
    /**
     * Checks whether a page has an avatar
     * @return boolean
     */
    public function has_avatar()
    {
        $avatar = $this->get_avatar_url();
        if (strpos($avatar, '/pages/') !== FALSE) {
            return (TRUE);
        }
        return (FALSE);
    }
    ...
    /*
     * Deletes the page's avatar image, including the original and small versions
     */
    ...
    /*
     * Moves an uploaded avatar file into the page's directory, renaming and converting
     * the file to .jpg
    ...
    public function finalize_move_avatar_file()
    {
        ...
        update_post_meta($this->id,'peepso_page_avatar_hash', $avatar_hash);
        ...
        if (file_exists($src_thumb) && file_exists($src_full) && file_exists($src_orig)) {
            ...
            if(class_exists('PeepSoSharePhotos')) {
                ...
                do_action('peepso_pages_after_change_avatar', $this->get('id'), $dest_thumb, $dest_full, $dest_orig);
            }
        }
    }
     ...
     * @return string href value for the page's cover photo
     */
    public function get_cover_url()
    {
        $cover = $this->get_cover_default();

        $page_cover_photo = get_post_meta($this->id, 'page_cover_photo', TRUE);
        if (!empty($page_cover_photo)) {
            $cover = $page_cover_photo;
        }
    ...
    /**
     * Get current page's default cover image.
     *
     * @return string
     */
    public function get_cover_default()
    {
        return PeepSo::get_asset('images/cover/page-default.png');
    }

    /*
     * Check if page has uploaded a custom cover photo
    ...
    /**
     * Display the current page's cover photo position percentage.
     */
    ...
    public function move_cover_file($src_file)
    {
        ...
        update_post_meta($this->id,'peepso_page_cover_hash', $hash);
        ...
        update_post_meta($this->id, 'page_cover_photo', $this->get_image_url() . $hash . '-cover.jpg');
        ...
        if(class_exists('PeepSoSharePhotos')) {
            ...
            do_action('peepso_pages_after_change_cover', $this->get('id'), $dest_file);
        }

    }

    /*
     * Deletes the page's cover photo and removes the database entry
     */
    public function delete_cover_photo($cover_hash = '')
    {
        ...
        if (!$cover_hash) {
            $cover_hash = get_post_meta($this->id, 'peepso_page_cover_hash', TRUE);
        ...
        delete_post_meta($this->id, 'page_cover_photo');
        delete_post_meta($this->id, 'peepso_cover_position_x');
        delete_post_meta($this->id, 'peepso_cover_position_y');
    ...
    /*
     * Checks for and creates the page's image directory
    ...
    /*
     * return directory where page's images are located
     * @param int $id User id to retrieve directory
     * @return string directory where page's images are located
     */
    // @todo DRY - repeated code
    public function get_image_dir()
    {
        // wp-content/peepso/pages/{user_id}/
//		$dir = WP_CONTENT_DIR . DIRECTORY_SEPARATOR . 'peepso' . DIRECTORY_SEPARATOR .
//			'pages' . DIRECTORY_SEPARATOR . $this->id . DIRECTORY_SEPARATOR;
        $dir = PeepSo::get_peepso_dir() . 'pages' . DIRECTORY_SEPARATOR . $this->id . DIRECTORY_SEPARATOR;
    ...
    {
//		$dir = WP_CONTENT_URL . '/peepso/pages/' . $this->id . '/';
        $dir = PeepSo::get_peepso_uri() . 'pages/' . $this->id . '/';
        return ($dir);
    }
