(function ($, factory) {
	var PsPagePageMembers = factory($);
	var ps_page_page_members = new PsPagePageMembers('.ps-js-page-members');
})(jQuery, function ($) {
	function PsPagePageMembers() {
		PsPagePageMembers.super_.apply(this, arguments);
		$($.proxy(this.init_page, this));
	}

	// inherit from `PsPageAutoload`
	peepso.npm.inherits(PsPagePageMembers, PsPageAutoload);

	peepso.npm.objectAssign(PsPagePageMembers.prototype, {
		...
		init_page: function () {
			...
			peepso.observer.addAction(
				...
				function (page_id, member_count) {
					...
					if (page_id === +peepsopagesdata.page_id) {
						$label = $('.ps-js-pending-count[data-id=' + page_id + ']');
			...
			peepso.observer.addAction(
				...
				function (page_id, member_count) {
					if (page_id === +peepsopagesdata.page_id) {
						$label = $('.ps-js-invited-count[data-id=' + page_id + ']');
						$label.html(member_count);
					}
				},
				10,
		...
		_search_url: 'pageusersajax.search',
		...
		_search_render_html: function (data) {
			var template = peepso.template(peepsopagesdata.memberItemTemplate),
				html = '',
				i;
			...
			$actions.each(
				$.proxy(function (index, elem) {
					...
					this._fetch_passive_actions(user_id)
						.done(function (data) {
							data = $.extend(data, {
								id: this._search_params.page_id,
		...
		_fetch_passive_actions: function (user_id) {
			var params = {
				page_id: this._search_params.page_id,
				passive_user_id: user_id
			};
			...
			return $.Deferred(
				$.proxy(function (defer) {
					peepso.getJson(
						'pageuserajax.member_passive_actions',
						params,
						$.proxy(function (response) {
