...
import peepsodata, { page as pageData } from 'peepsodata';
...
const MODULE_ID = +pageData.module_id;
const IMG_COVER_DEFAULT = pageData.img_cover_default;
const COVER_NONCE = pageData.cover_nonce;
const COVER_UPLOAD_URL = `${peepsodata.ajaxurl_legacy}pageajax.cover_upload?cover`;
const COVER_TEXT_ERROR_FILETYPE = pageData.text_error_filetype;
const COVER_TEXT_ERROR_FILESIZE = pageData.text_error_filesize;

let hasCover = +pageData.has_cover;
let hammertime;

$(function () {
	let $container = $('.ps-js-focus--page .ps-js-cover');
	if (!$container.length) {
		return;
	}
	...
	function uploadInit() {
		...
		if ($file.psFileupload) {
			$file.psFileupload({
				formData: { page_id: GROUP_ID, module_id: MODULE_ID, _wpnonce: COVER_NONCE },
				dataType: 'json',
				...
				add(e, data) {
					...
					if (json.success) {
						...
						hooks.doAction('page_cover_updated', GROUP_ID, imgCover);
					} else if (json.errors) {
						alert(json.errors);
					}
				}
	...
	function remove(e) {
		...
		let popup = dialog(pageData.template_cover_remove).show();
		popup.$el.on('click', '.ps-js-cancel', () => popup.hide());
		popup.$el.on('click', '.ps-js-submit', () => {
			popup.hide();
			$coverImage.css('opacity', 0.5);

			let params = { page_id: GROUP_ID, _wpnonce: COVER_NONCE };
			ajax.post('pageajax.cover_delete', params).then(json => {
				if (json.success) {
					$reposition.hide();
					$rotateLeft.hide();
					$rotateRight.hide();
					...
					hooks.doAction('page_cover_updated', GROUP_ID, IMG_COVER_DEFAULT);
				}
			});
		});
	}
	...
	function repositionConfirm(e) {
		...
		ajax.post('pageajax.cover_reposition', params); // Optimistic call.
	}
	...
	function rotate(e) {
		...
		let params = $.extend(
			...
			{ page_id: GROUP_ID, _wpnonce: COVER_NONCE }
		);

		ajax.post('pageajax.cover_rotate', params).then(json => {
			if (json.success) {
				let imgCover = json.data.image_url;
	...
	let _docWidth;
	hooks.addAction('browser_resize', 'page_cover', dimension => {
	...
	hooks.addAction('page_cover_updated', 'page_page', (id, imageUrl) => {
		if (+id === GROUP_ID) {
			if (imageUrl.indexOf(`/pages/${id}/`) > -1) {
				// Custom cover.
