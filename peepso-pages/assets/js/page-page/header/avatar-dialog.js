...
const IMG_AVATAR_DEFAULT = pageData.img_avatar_default;
const AVATAR_NONCE = pageData.avatar_nonce;
const AVATAR_UPLOAD_URL = `${peepsodata.ajaxurl_legacy}pageajax.avatar_upload?avatar`;
const AVATAR_TEXT_ERROR_FILETYPE = pageData.text_error_filetype;
const AVATAR_TEXT_ERROR_FILESIZE = pageData.text_error_filesize;

let hasAvatar = +pageData.has_avatar,
	imgAvatar,
	imgAvatarOriginal,
	popup,
	$container,
...
function init() {
	...
	// Set initial state.
	update.apply(null, hasAvatar ? [pageData.img_avatar, pageData.img_avatar_original] : []);
...
function uploadInit() {
	...
	if ($file.psFileupload) {
		$file.psFileupload({
			formData: { page_id: GROUP_ID, module_id: MODULE_ID, _wpnonce: AVATAR_NONCE },
			dataType: 'json',
...
function remove(e) {
	...
	let params = { page_id: GROUP_ID, module_id: MODULE_ID, _wpnonce: AVATAR_NONCE };
	ajax.post('pageajax.avatar_delete', params).then(json => {
		if (json.success) {
			...
			hooks.doAction('page_avatar_updated', GROUP_ID, IMG_AVATAR_DEFAULT);
		}
	});
}
...
function finalize(e) {
	...
	if (imgIsTemporary) {
		...
		finalizing = true;
		ajax.post('pageajax.avatar_confirm', params)
			.then(json => {
				if (json.success) {
					let avatar = json.data && json.data.image_url;
					...
					imgIsTemporary = false;
					$done.attr('disabled', 'disabled');
					hooks.doAction('page_avatar_updated', GROUP_ID, avatar);
				}
			})
			.always(function () {
				finalizing = false;
...
function cropConfirm(e) {
	...
	ajax.post('pageajax.avatar_crop', params).then(json => {
		if (json.success) {
...
function rotate(e) {
	...
	ajax.post('pageajax.avatar_rotate', params).then(json => {
		if (json.success) {
			let avatar = json.data && json.data.image_url,
				original = json.data && json.data.orig_image_url;
...
export default function () {
	if (!popup) {
		popup = dialog(pageData.template_avatar, { wide: true, destroyOnClose: false }).show();
		init();
	}
