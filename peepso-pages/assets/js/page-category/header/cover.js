...
import peepsodata, { page_category as categoryData } from 'peepsodata';
...
const COVER_UPLOAD_URL = `${peepsodata.ajaxurl_legacy}pagecategoryajax.cover_upload?cover`;
...
$(function () {
	let $container = $('.ps-js-focus--page-category .ps-js-cover');
	if (!$container.length) {
		return;
	}
	...
	function uploadInit() {
		let name = 'page-category-cover-upload',
			accept = 'image/*',
			...
			$file.psFileupload({
				...
				add(e, data) {
					...
					if (json.success) {
						...
						hooks.doAction('page_cover_updated', CATEGORY_ID, imgCover);
					} else if (json.errors) {
						alert(json.errors);
					}
				}
	...
	function remove(e) {
		...
		popup.$el.on('click', '.ps-js-submit', () => {
			...
			ajax.post('pagecategoryajax.cover_delete', params).then(json => {
				if (json.success) {
					$reposition.hide();
					$rotateLeft.hide();
					$rotateRight.hide();
					...
					hooks.doAction('page_cover_updated', CATEGORY_ID, IMG_COVER_DEFAULT);
				}
			});
		});
	}
	...
	function repositionConfirm(e) {
		...
		ajax.post('pagecategoryajax.cover_reposition', params); // Optimistic call.
	}
	...
	function rotate(e) {
		...
		ajax.post('pagecategoryajax.cover_rotate', params).then(json => {
			if (json.success) {
				let imgCover = json.data.image_url;
	...
	hooks.addAction('page_cover_updated', 'page_page_category', (id, imageUrl) => {
		if (+id === CATEGORY_ID) {
			if (imageUrl.indexOf(`/page-categories/${id}/`) > -1) {
				// Custom cover.
