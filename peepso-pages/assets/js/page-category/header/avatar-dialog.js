...
const AVATAR_UPLOAD_URL = `${peepsodata.ajaxurl_legacy}pagecategoryajax.avatar_upload`;
...
function uploadInit() {
	let name = 'page-category-avatar-upload',
		accept = 'image/*',
...
function remove(e) {
	...
	ajax.post('pagecategoryajax.avatar_delete', params).then(json => {
		if (json.success) {
			...
			hooks.doAction('page_category_avatar_updated', CATEGORY_ID, IMG_AVATAR_DEFAULT);
		}
	});
}
...
function finalize(e) {
	...
	if (imgIsTemporary) {
		...
		finalizing = true;
		ajax.post('pagecategoryajax.avatar_confirm', params)
			.then(json => {
				if (json.success) {
					let avatar = json.data && json.data.image_url;
					...
					imgIsTemporary = false;
					$done.attr('disabled', 'disabled');
					hooks.doAction('page_category_avatar_updated', CATEGORY_ID, avatar);
				}
			})
			.always(function () {
				finalizing = false;
...
function cropConfirm(e) {
	...
	ajax.post('pagecategoryajax.avatar_crop', params).then(json => {
		if (json.success) {
...
function rotate(e) {
	...
	ajax.post('pagecategoryajax.avatar_rotate', params).then(json => {
		if (json.success) {
			let avatar = json.data && json.data.image_url,
				original = json.data && json.data.orig_image_url;
