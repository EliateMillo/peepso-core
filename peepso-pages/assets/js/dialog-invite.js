...
const TEMPLATE = window.peepsopagesdata && window.peepsopagesdata.dialogInviteTemplate;

(function ($, factory) {
	var PsPageDialogInvite = factory('PsPageDialogInvite', $);

	peepso.pages || (peepso.pages = {});
	peepso.pages.dlgInvite = function (pageId) {
		let popup = new PsPageDialogInvite(pageId);
		popup.popup.show();
	};
		...
		fetch: function (params) {
			return $.Deferred(
				$.proxy(function (defer) {
					peepso.getJson(
						'pageusersajax.search_to_invite',
						params,
		...
		render: function (data) {
			...
			if (data.users && data.users.length) {
				for (var i = 0; i < data.users.length; i++) {
					html += this.itemTemplate($.extend({ page_id: this.pageId }, data.users[i]));
				}
