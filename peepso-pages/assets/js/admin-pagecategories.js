(function($) {
	var $container = $('.ps-js-page-categories-container');
	...
	function setProp(propType, params, callback) {
		...
		var url = 'pageCategoryAjax.set_' + propType;
	...
	$container.on(
		...
		_.throttle(function() {
			...
			url = 'pageCategoryAjax.set_' + ($cbx.data('prop-type') === 'meta' ? 'meta' : 'prop');
			params = {
			...
			peepso.postJson(url, params, function(json) {
				...
				if (json.success) {
					...
					if (params.prop === 'validation' && params.key === 'required') {
						var $mark = $('#page-category-' + params.id + '-required-mark');
						if (checked) {
	...
	(function($container) {
		var inputSelector = [
			'.ps-js-page-category input[type=text][data-prop-name]',
			'.ps-js-page-category textarea[data-prop-name]'
		].join(', ');
		...
		$container.on('click', '.ps-js-page-category .ps-js-cancel', function() {
			var $ct = $(this).closest('.ps-js-fieldconf'),
				$btns = $ct.find('.ps-js-btn'),
		...
		$container.on('click', '.ps-js-page-category .ps-js-save', function() {
			var $ct = $(this).closest('.ps-js-fieldconf'),
				$btns = $ct.find('.ps-js-btn'),
				$input = $ct.find('input[type=text][data-prop-name], textarea[data-prop-name]'),
			...
			url =
				'pageCategoryAjax.set_' + ($input.data('prop-type') === 'meta' ? 'meta' : 'prop');
	...
	$container.on('click', '.ps-js-page-category .ps-js-generate-slug', function(e) {
		e.preventDefault();
		e.stopPropagation();
	...
	$container.sortable({
		...
		update: _.throttle(function() {
			var fields = [];
			$('.ps-js-page-categories-container .postbox').each(function() {
				fields.push($(this).data('id'));
			});

			peepso.postJson(
				'pageCategoryAjax.set_order',
				{ page_category: JSON.stringify(fields) },
				function(json) {}
			);
		}, 3000)
	});
	...
	// toggle a field
	$container.on('click', '.ps-js-page-category-toggle', function() {
		var $btn = $(this),
			$el = $btn.closest('.postbox'),
			$field = $el.find('.ps-js-page-category'),
			id = $el.data('id');
	...
	// toggle a field
	$container.on('click', '.ps-js-page-category-title', function(e) {
		if (e.target === e.currentTarget) {
			$(this)
				.closest('.ps-postbox__title')
				.find('.ps-js-page-category-toggle')
				.click();
		}
	});
	...
	// toggle expand all page categories
	$('.ps-js-page-categories-expand-all').on('click', function() {
		$container.find('.ps-js-page-category').slideDown('fast', function() {
			toggleAllCallback(1);
		});
	});

	// toggle collapse all page categories
	$('.ps-js-page-categories-collapse-all').on('click', function() {
		$container.find('.ps-js-page-category').slideUp('fast', function() {
			toggleAllCallback(0);
		});
	});

	var toggleAllCallback = _.debounce(function(status) {
		var $fields = $('.ps-js-page-categories-container').children('.postbox');
	...
	function updateToggleAllButton(status) {
		var $btn = $('.ps-js-page-category-toggle-all'),
		...
		if (typeof status === 'undefined') {
			...
			visible = 0;
			status = 0;
			$('.ps-js-page-categories-container')
				.find('.ps-js-page-category')
				.each(function() {
					len++;
					if ($(this).is(':visible')) {
						visible++;
	...
	var updateFieldVisibility = _.debounce(function(id, status) {
		...
		updateFieldXHR[id] = peepso.postJson(
			'pageCategoryAjax.set_admin_box_status',
	...
	$('.ps-js-page-categories-new').on('click', function() {
		peepso.postJson('pageCategoryAjax.create', {}, function(response) {
			if (response.success) {
	...
	// edit title handler
	$container.on('click', '.ps-js-page-category-title-text', function() {
		var $ct = $(this).closest('.ps-postbox__title'),
			$btn = $ct.find('.fa-edit');
		$btn.click();
	});
	...
	$container.on('click', '.ps-postbox__title .ps-js-save', function() {
		...
		url = 'pageCategoryAjax.set_' + ($input.data('prop-type') === 'meta' ? 'meta' : 'prop');
		params = {
			id: $input.data('parent-id') || undefined,
			prop: $input.data('prop-name') || undefined,
			key: $input.data('prop-key') || undefined,
		...
		peepso.postJson(url, params, function(json) {
			...
			if (json.success) {
				...
				$('#page-category-' + params.id + '-box-title').html(params.value);
			} else {
				// TODO
			}
		});
	...
	$container.on('click', '.ps-js-page-category-delete', function(e) {
		e.preventDefault();
		e.stopPropagation();
		if (confirm('Are you sure want to delete this category?')) {
			var id = $(this).data('id');
			peepso.postJson('pageCategoryAjax.delete', { id: id }, function(response) {
				if (response.success) {
