...
class PeepSoGroupsPlugin
{
    ...
    private function __construct()
    {
        ...
        add_action('init', function(){
            $path = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'language' . DIRECTORY_SEPARATOR . 'groupso-' . determine_locale() . '.mo';
            load_textdomain('groupso', $path);
        });
        ...
        if(self::ready()) {
            ...
            add_filter('peepso_dashboard_most_recent_content', function($post_id, $content){
                if(strlen(get_post_meta($post_id, self::JOIN_STREAM_META, TRUE))) {
                    $content = PeepSo::get_option('groups_join_post_action_text_other', __('joined a group','groupso'));
                    if (empty($content)) {
                        $content = __('joined a group','groupso');
                    }
                }
    ...
    public function init()
    {
        ...
        if (is_admin()) {
            ...
            add_filter('peepso_postbox_options', 			array(&$this, 'filter_postbox_options'), 17, 2);
            add_filter('peepso_postbox_options_post_to_options', array(&$this, 'filter_postbox_options_post_to_options'), 15);
            ...
            add_action('peepso_groups_new_post', function($group_id, $post_id) {
                ...
                if(PeepSo::get_option_new('postbox_anon_enabled')) {
                    ...
                    if(strlen($anon_op) && intval($anon_op) == $post_author) {
                        $post_author = PeepSo3_anon::get_instance()->anon_id;
                    }
                }

                $from_first_name = PeepSoUser::get_instance($post_author)->get_firstname();
                ...
                if ($post->post_status != 'pending') {
                    foreach($followers as $follower_id) {
                        if($follower_id == $post_author || (strlen($anon_op) && intval($anon_op) == $follower_id) || $block->is_user_blocking($follower_id, $post_author)) { continue; }

                        $PeepSoNotifications->add_notification_new($post_author, $follower_id, $message, $args,'groups_new_post', self::MODULE_ID, $post_id);
                    }
                }
            ...
            add_filter('peepso_filter_opengraph_tags', array(&$this, 'filter_opengraph_tags'), 10, 2);
            ...
            add_filter('peepso_save_notifications', function($field) {
                ...
                if(!is_array($field) && strstr( $field, 'group_')) {
                    $field=explode('_', $field);
    ...
    public function filter_postbox_options($options, $params = [])
    {
        $is_group_view = FALSE;
        $category_id = FALSE;
        ...
        $anon_enabled = PeepSo::get_option_new('postbox_anon_enabled');
        if ($is_group_view && $anon_enabled) {
            $options['post_to']['icon'] = 'pso-i-users-alt';
            $options['post_to']['label'] = __('This group', 'groupso');
        } else if ($is_group_view) {
            unset($options['post_to']);
        }

        // Do not show post_to dropdown on the group page.
        if ($category_id) {
            $options['post_to']['icon'] = 'pso-i-users-alt';
            $options['post_to']['label'] = __('Select a group', 'groupso');
        }
    ...
    public function filter_postbox_options_post_to_options($opts = [])
    {
        ...
        if ($category_id) {
            $opts = [];
            $opts['groups'] = PeepSoTemplate::exec_template('groups', 'postbox-post-to', ['category_id' => $category_id], TRUE);
        } elseif ($is_group_view) {
            $opts['profile'] = PeepSoTemplate::exec_template('groups', 'postbox-post-to-group', NULL, TRUE);
            unset($opts['groups']);
        } else {
            $opts['groups'] = PeepSoTemplate::exec_template('groups', 'postbox-post-to', NULL, TRUE);
        }
    ...
    public function filter_activity_stream_title($title, $post, $action)
    {
        ...
        if ($update_post_title) {
            ...
            if(PeepSo::get_option_new('postbox_anon_enabled')) {
                $anon_op = get_post_meta($post->ID, PeepSo3_anon::META_POST_ANON_OP, TRUE);
                if(strlen($anon_op) && intval($anon_op) == $post->post_author) {
                    $post_author = PeepSo3_anon::get_instance()->anon_id;
                }
            }
    ...
    public  function enqueue_scripts()
    {
        ...
        wp_enqueue_script('peepso-groups',
            ...
            array('peepso-bundle', 'peepso-page-autoload', 'peepso-groups-create'), self::PLUGIN_VERSION, TRUE);
    ...
    public function filter_opengraph_tags($tags, $url) {
        if ($url->_shortcode == 'peepso_groups' && $url->get(1)) {
            $PeepSoGroup = new PeepSoGroup($url->get(1));
            if($PeepSoGroup->id) { 
                $tags['title'] .= ' - ' . trim(strip_tags($PeepSoGroup->get('name')));
                $tags['description'] = trim(strip_tags($PeepSoGroup->get('description')));
                $tags['image'] = $PeepSoGroup->get_avatar_url();
                $tags['url'] = $PeepSoGroup->get_url();   
            }
        }
