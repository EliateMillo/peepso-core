...
(function (factory) {
	...
	PsMessages.prototype.new_message = function (user_id, flag, btn, opts) {
		...
		// show the new-message page if setting is enabled
		if (+messagesData.new_message_page_enabled) {
			let url = messagesData.new_message_page_url;
			let params = [];

			if (user_id) params.push(`id=${user_id}`);
			if ('object' === typeof opts)
				Object.keys(opts).forEach(key => params.push(`${key}=${opts[key]}`));
